...
public:
    ...
    ~DSString();  // destructor
    DSString &operator=(const DSString &); // assignment operator taking DSString&
    DSString &operator=(DSString *); // assignment operator taking DSString
    DSString &operator=(const char *); // assignment operator for c_str
...
//    bool operator<(const DSString &) const;
    ...
    char getData(int value) const { return data[value]; }
};
